...
export default function Comments(props){
    ...
    const [commentName,setCommentName]=useState(props.dataForComment.name);
    const [commentBody,setCommentBody]=useState(props.dataForComment.body);

    //Handler Function to Handle Comment Name
    ...
    const deleteComment=()=>
    {
        ...
        {
            return (
                <Button variant="outline-danger" onClick={handleDeleteBtn} >Delete</Button>);
        }
        return (<></>);
    }
    ...
    const editComment=()=>
    {
        ...
        {
            return (
                <Button variant="outline-info" onClick={handleEditBtn} >EDIT</Button>);
        }
        return (<></>);
    }
    //After Pressing Edit Comment, update Comment Button Appears
    //When UpdateButton is Pressed this function is called 
    //Which updates the local state of comments object
    const updateComment=()=>
    {
        let userEmail=findEmail(userid);
        ...
        if(props.dataForComment.email===userEmail)
        {
            return (<Button variant="outline-warning" onClick={handleUpdateBtn}>UPDATE</Button>);
        }
    }
    ...
    {
        updateEditFlag(true);
        props.dataForComment.name=commentName;
    ...
    const renderCommentInput=()=>
    {
        return(<>
            <div className="row justify-content-center comment-input-div">
                ...
                value={commentName}
                onChange={handleCommentName}></input>
                </div>
            </div>
    ...
    <>
        <div className="row justify-content-center upper-div">
            ...
            <div className="wrapping-div col-8 justify-content-center">
                    ...
                    {editFlag?(
                        renderCommentText()) :(renderCommentInput())
                    }
